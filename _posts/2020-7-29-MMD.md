...
Consider the case where kernel $$k$$ is continuous and bounded. Thus one has $$\int\int k(x,y)dP(x)dQ(y)<\infty$$, and therefore one can use Fubini theorem. We can also define MMD in a different way to see its relation with other metrics
